...
export function HeroSection() {
  ...
  useEffect(() => {
    const ctx = gsap.context(() => {
      ...
      gsap.from(".hero-headline", { opacity: 0, scale: 0.9, duration: 0.8, delay: 0.2, ease: "power3.out" })
      gsap.from(".hero-sub", { opacity: 0, y: 20, duration: 0.6, delay: 0.4, ease: "power3.out" })
      gsap.from(".hero-cta", { opacity: 0, y: 20, duration: 0.5, delay: 0.6, stagger: 0.15, ease: "power3.out" })
      gsap.from(".hero-stats", { opacity: 0, y: 30, duration: 0.6, delay: 0.8, ease: "power3.out" })
    }, sectionRef)
    return () => ctx.revert()
  }, [])

  return (
    <section ref={sectionRef} className="relative min-h-[70vh] sm:min-h-[80vh] md:min-h-[90vh] flex items-center overflow-hidden bg-background">
      ...
      <div className="container mx-auto px-4 relative z-10 py-16 sm:py-20 md:py-32">
        <div className="flex flex-col items-center text-center max-w-4xl mx-auto gap-4 sm:gap-6 md:gap-8">
          ...
          <h1 className="hero-headline text-4xl sm:text-5xl md:text-7xl lg:text-8xl font-bold leading-tight text-foreground">
            <span className="text-primary">Cyra&apos;s</span> Preloved
          </h1>

          <p className="hero-sub text-base sm:text-lg md:text-xl text-muted-foreground max-w-2xl leading-relaxed">
            100% asli, terkurasi dengan cinta, harga spesial. Koleksi pribadi Cyra untuk kamu yang
            mengutamakan kualitas.
          </p>

          <div className="hero-cta flex flex-col sm:flex-row items-center justify-center gap-3 sm:gap-4 w-full sm:w-auto">
            <Link href="/products" className={buttonVariants({ size: "lg", className: "w-full sm:w-auto" })}>
              Lihat Koleksi
          ...
          <div className="hero-stats flex flex-wrap items-center justify-center gap-4 sm:gap-6 md:gap-8 pt-2 sm:pt-4">
            {[
              ...
              <div key={i} className="flex items-center gap-2 sm:gap-3">
                ...
                <div className="text-left">
                  <span className="block text-xs sm:text-sm font-semibold text-foreground">{stat.label}</span>
                  <span className="block text-[10px] sm:text-xs text-muted-foreground">{stat.sub}</span>
                </div>
              </div>
            ))}
          </div>
